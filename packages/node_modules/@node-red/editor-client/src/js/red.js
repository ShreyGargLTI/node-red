...
var RED = (function() {
    ...
    function setSessionsUser(data){
        var str= data;
        var str= str.split('?')[1];
        if(str != null){
            var dataarr = str.split(',')
            sessionStorage.setItem("gituserId",dataarr[0]);
            sessionStorage.setItem("gituserEmail",dataarr[1]);
            sessionStorage.setItem("gitremoteUrl",dataarr[2]);
            sessionStorage.setItem("nodeUserLogged", dataarr[0]);
            sessionStorage.setItem("devtoken", dataarr[3]);
            sessionStorage.setItem("podId", dataarr[4]);
            var url = 'https://ltimosaicthings-dev.azurewebsites.net/workbench/task/validate-dev-token';
            if(dataarr[5] == "prod"){
                url = 'https://ltimosaicthings.azurewebsites.net/workbench/task/validate-dev-token';
            }
            sessionStorage.setItem("url", url);

            var project = dataarr[2];
            var lastslashindex = project.lastIndexOf('/');
                 project = project.substring(lastslashindex  + 1);
            var projarr =  project.split('.');
            sessionStorage.setItem("projectName", projarr[0]); 
            return true;
        }else{
            return false;
        }
